    ...
    `;
      ...
      <tr>
        ...
        <td class="text-center">
          <select id="alloc-type-${idx}" class="form-select" ${
        ...
        <div style="display:flex; align-items:center; justify-content:center; gap:8px;">
          ...
          ${
            lockRevenue
              ...
              : `<button type="button" class="btn btn-sm" data-bs-toggle="modal" data-bs-target="#hesoModal-${idx}" title="${
                  lang === "vi" ? "Chỉnh hệ số" : "Edit coefficient"
                }">
              ...
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title">${
                    lang === "vi"
                      ? `Cấu hình hệ số - ${p.name}`
                      : `Coefficient Configuration - ${p.name}`
                  }</h5>
                ...
                <div class="modal-body">
                  ...
                  <div style="display:flex; align-items:center; gap:8px; margin-bottom:10px;">
                    ...
                    ${productTypeHtml}
                    <input id="product-type-value-${idx}" class="form-control" style="width:120px;" readonly value="${escapeHtml(
        p.productTypeValue || 0
      )}%">
                  </div>
                  ...
                  <div style="display:flex; align-items:center; gap:8px; margin-bottom:10px;">
                    <label style="min-width:150px;">${
                      lang === "vi" ? "Loại SPDV" : "Product/Service Type"
                    }</label>
                    ${spdvHtml}
                    <input id="spdv-type-value-${idx}" class="form-control" style="width:120px;" readonly value="${escapeHtml(
        p.spdvTypeValue || 0
      )}%">
                  </div>
                  ...
                  <div style="display:flex; align-items:center; gap:8px; margin-bottom:10px;">
                    ...
                    ${regionHtml}
                    <script>
                      (function(){ try{ handleRegionChange(${idx}, "${escapeAttr(
                          market
                        )}"); validateSelections(${idx}); }catch(e){console.warn(e);} })();
                    </script>
                    <input id="region-value-${idx}" class="form-control" style="width:120px;" readonly value="${escapeHtml(
                          p.regionValue || 0
                        )}%">
                  </div>
                  `
                  ...
                  <div style="display:flex; align-items:center; gap:8px; margin-top:15px;">
                    ...
                    <input id="total-coef-${idx}" class="form-control" style="width:120px;" readonly value="${
        p.coefficient || ""
      }">
                  </div>
                </div>
                <div class="modal-footer">
                  ...
                  <button type="button" class="btn btn-primary" onclick="saveHeSo(${idx})">${
        lang === "vi" ? "Save" : "Lưu"
      }</button>
                </div>
              </div>
    ...
    <table class="table table-bordered">
      <thead>
        <tr>
          ...
          <th class="text-center">${
            lang === "vi" ? "Loại phân bổ" : "Allocation Type"
          }</th>
          <th class="text-center">${
            lang === "vi" ? "Số lần phân bổ" : "Allocation Count"
          }</th>
          <th class="text-center">${
            lang === "vi" ? "Hệ số phân bổ" : "Coefficient"
          }</th>
          <th class="text-center">${
            lang === "vi" ? "Ngày dự kiến" : "Forecast Date"
          }</th>
          <th class="text-center">${
            lang === "vi" ? "Ngày chính thức" : "Actual Date"
          }</th>
        </tr>
      </thead>
      <tbody>
...
function renderAllocationPreview() {
  ...
  previous.textContent =
    lang === "vi" ? "Phân bổ lần trước" : "Previous Allocation";
      ...
      const rows = rec.allocations
        .map((a, i) => {
          ...
          if (facDate instanceof Date) {
            ...
            actualCell = `${calcDate.getDate().toString().padStart(2, "0")}/${(
              calcDate.getMonth() + 1
            )
              .toString()
          ...
          return `
            <tr>
              ...
              <td id="actual-allocated-${rec.id}-${i}">${
            a.actualValue ? formatCurrency(a.actualValue, rec.currency) : ""
          }</td>
              ...
              <td id="acceptance-cell-${rec.id}-${i}">
                ...
                <button class="btn btn-link p-0 ms-1" title="Edit Acceptance Date" ${
                  lockRevenue ? "disabled" : ""
                }
              ...
              <table class="table mb-0 table-sm">
                <thead class="table-light">
                  <tr>
                    ...
                    <th>${
                      lang === "vi" ? "Phân bổ dự kiến" : "Forecast Allocation"
                    }</th>
                    <th>${
                      lang === "vi" ? "Ngày chính thức" : "Actual date"
                    }</th>
                    <th>${
                      lang === "vi" ? "Phân bổ thực tế" : "Actual Allocation"
                    }</th>
                    <th>${
